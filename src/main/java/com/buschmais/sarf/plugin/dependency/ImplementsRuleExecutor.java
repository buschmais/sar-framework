...
public class ImplementsRuleExecutor extends DependencyRuleExecutor<ImplementsRuleDescriptor> {
    ...
    public ImplementsRuleExecutor(XOManager xoManager, DependencyRepository dependencyRepository) {
        super(xoManager, dependencyRepository);
    }

    @Override
    protected Result<TypeDescriptor> getMatchingTypes(ImplementsRuleDescriptor rule) {
        return dependencyRepository.getAllInternalTypesImplementing(rule.getRule());
    }
}
